...
public class CTRETalonFX {
  ...
  private static final Logger logger = Logger.getLogger(CTRETalonFX.class.getName());

  public static void logFaults(TalonSRX CTRETalonFX) {
    runTest(() -> {
      String separator = "// ==========";
      Faults CTRE_0xF = new Faults();
      CTRETalonFX.getFaults(CTRE_0xF);
  
      if(CTRE_0xF.hasAnyFault()) {
        System.out.println("Faults detected on TalonFX ID " + CTRETalonFX.getDeviceID());
