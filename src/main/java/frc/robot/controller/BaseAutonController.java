...
public class BaseAutonController extends AbstractController {
     ...
     * This
     * will be used to calculate the chasis speed based on the robots pose and
     * velocity
    ...
    /**
     * Method that should be called when the autonomous state is initialized. This
     * is so that the overall
     * state can be restarted, should the autonomous be run multiple times.
    ...
    /**
     * Method to perform the actual traversal. This should run a single step. A step
     * will be defined as
     * a sequence of execution between some delta time from the previous execution.
    ...
    public void execute() {
        ...
        data.add(ABC.feet_to_meters(db.drivetrain.get(EDriveData.L_ACTUAL_POS_FT)));
        data.add(ABC.feet_to_meters(db.drivetrain.get(EDriveData.R_ACTUAL_POS_FT)));
        ...
        mPrevActualSpeed = new DifferentialDriveWheelSpeeds(actualSpeeds.leftMetersPerSecond,
                actualSpeeds.rightMetersPerSecond);
     ...
     * @param pDt                     The amount of time that has passed since the
     *                                last execution
    ...
    private void logDataToSmartDashboard(double dt, Trajectory.State sample,
            DifferentialDriveWheelSpeeds targetWheelSpeeds, DifferentialDriveWheelSpeeds actualSpeeds) {
